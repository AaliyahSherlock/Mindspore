...
# torch to mindspore not finished
# 有一个没有对应算子的
...
def f_score(inputs, target, beta=1, smooth=1e-5, threhold=0.5):
    ...
    if h != ht and w != wt:
        # 修改？ 现在没有对应的算子,不知道这样可不可以(希望找到一个能作双线性（bilinear）插值的mindspore算子)
    ...
    # 修改 来源：torch.softmax的原型格式是torch.nn.functional.softmax(pytorch文档说这个函数不大好使，建议我们换一个)，映射API为mindspore.ops.Softmax，作用是归一化
    softmax = ops.Softmax(axis=-1)
    temp_inputs = softmax(inputs.transpose(1, 2).transpose(2, 3).contiguous().view(n, -1, c))
    ...
    #   计算dice系数
    # --------------------------------------------#
    # 修改 来源：https://mindspore.cn/docs/zh-CN/r1.8/api_python/ops/mindspore.ops.gt.html#mindspore.ops.gt
    temp_inputs = ops.gt(temp_inputs, threhold).float()
    # temp_inputs = torch.gt(temp_inputs, threhold).float()
    # 修改 来源：https://mindspore.cn/docs/zh-CN/r1.8/api_python/ops/mindspore.ops.ReduceSum.html#mindspore.ops.ReduceSum
    op = ops.ReduceSum(keep_dims=False)
    tp = op(temp_target[..., :-1] * temp_inputs, [0, 1])
    fp = op(temp_inputs, [0, 1]) - tp
    fn = op(temp_target[..., :-1], [0, 1]) - tp
    # tp = torch.sum(temp_target[..., :-1] * temp_inputs, axis=[0, 1])
    # fp = torch.sum(temp_inputs, axis=[0, 1]) - tp
    # fn = torch.sum(temp_target[..., :-1], axis=[0, 1]) - tp

    score = ((1 + beta ** 2) * tp + smooth) / ((1 + beta ** 2) * tp + beta ** 2 * fn + fp + smooth)
    # 修改 来源：https://mindspore.cn/docs/zh-CN/r1.8/api_python/ops/mindspore.ops.ReduceMean.html#mindspore.ops.ReduceMean
    score = ops.ReduceMean(score)
